...
if not all_data:
    ...
    plt.plot(x, hourly_mean.values, label='Predicted Mean (Apr 1 – May 31)', marker='o')
    plt.fill_between(x,
    ...
    if actual_all:
        ...
        plt.plot(actual_mean.index.values, actual_mean.values, label='Observed (June 1)', linestyle='--', marker='x', color='red')
    
    plt.xlabel('Hour of Day')
    plt.ylabel('Electricity Usage (kWh)')
    plt.title('Predicted vs Observed Electricity Usage on June 1')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("predicted_vs_observed_june1.png")
    plt.close()
