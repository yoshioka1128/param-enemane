...
dirs = ['raw', 'raw2']
all_data = []
target_dates = pd.date_range('2019-04-01', '2019-05-31').strftime('%Y/%m/%d')
predict_date = '2019/06/01'
...
for _, row in df_kanto.iterrows():
    ...
    if not {'計測日', '計測時間', '電力使用量_全体'}.issubset(df.columns):
        continue
    ...
    pivot = df_range.pivot(index='計測日', columns='計測時間', values='電力使用量_全体')
